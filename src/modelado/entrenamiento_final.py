...
def redactar_model_card(n_arboles_reparto: int, n_arboles_almacen: int, test_metricas: dict, peso_zona_reparto: float, ejemplos_md: list[str], n_rutas_train: int, n_rutas_test: int) -> str:
    return "\n".join(
        [
            ...
            f"Generada automaticamente por `entrenamiento_final.py` el {datetime.now(UTC).date()}",
            ...
            "```text",
            f"reparto   CatBoost, {n_arboles_reparto} arboles, con to_zone_id nativa",
            f"almacen   LightGBM, {n_arboles_almacen} arboles",
            "```", "",
            ...
            f"train   {n_rutas_train:,} rutas (80%)",
            f"test    {n_rutas_test:,} rutas (20%, nunca visto en el ajuste)",
            "",
            ...
            f"reparto   MAE {test_metricas['reparto']['mae_min']:.4f} min   R2 {test_metricas['reparto']['r2']:.4f}   (n={test_metricas['n_test_reparto']:,})",
            f"almacen   MAE {test_metricas['almacen']['mae_min']:.4f} min   R2 {test_metricas['almacen']['r2']:.4f}   (n={test_metricas['n_test_almacen']:,})",
            "```", "",
            "## Variables mas influyentes (TreeSHAP)", "",
            "`segment_distance_km` domina en las dos poblaciones. `to_zone_id` aporta",
            f"{peso_zona_reparto:.1f}% de la magnitud media de SHAP en reparto; es la razon de",
