...
arquitectura ganar y por qué, pero al re-ejecutar `src/modelado/bakeoff/*.py` en este repositorio
las cifras exactas pueden variar ligeramente, porque ahora el bake-off compara arquitecturas solo
dentro del 80% de `train` (ver `docs/modelado/model_card.md` para la razón de este cambio).

## La tabla

```text
Arquitectura                              MAE (min)   R2       Nota
────────────────────────────────────────────────────────────────────────────────────────
1. Baseline (recta por poblacion)          0,4473     0,9192   el suelo a batir
2. LightGBM unico (sin separar poblacion)  0,3937     0,9121   mejora MAE, EMPEORA R2
3. Hibrido (2 especialistas, sin zona)     0,3800     0,9349   arregla el R2
...
Un único modelo con `is_depot_segment` como variable normal (no como separador estructural de
dos especialistas) reduce el error medio absoluto pero empeora la varianza explicada: el modelo
...
(cifra exacta en `artifacts/modelo/model_card.md`, recalculada por este repositorio).
...
Cada uno lee `modelado.datos.cargar_train()` (el 80% de las rutas, nunca el test set) y escribe
su informe a `artifacts/modelado/reports/`, sus figuras a `artifacts/modelado/figuras/`, y registra
en el mismo MLflow (`artifacts/modelado/mlflow/`) que `uv run sdai-train`.
